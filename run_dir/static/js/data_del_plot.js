...
function build_series(data, key, name, view_type, filter_inst_type){
    ...
    for (d in data){
        ...
        var date_close = data[d].close_date;
        if (data[d].sequencing_platform == null){
            continue;
        }else if (data[d].sequencing_platform.includes('NovaSeq') && filter_inst_type.includes('NovaSeq')){
            continue;
        }else if (data[d].sequencing_platform.includes('MiSeq') && filter_inst_type.includes('MiSeq')){
            continue;
        }else if (data[d].sequencing_platform.includes('NextSeq') && filter_inst_type.includes('NextSeq')){
            continue;
        }else if (data[d].sequencing_platform.includes('HiSeq') && filter_inst_type.includes('HiSeq')){
            continue;
        }
        if (view_type == 'sequencing_platform'){
            if (data[d].sequencing_platform.includes('NovaSeq')){
                series_name = "NovaSeq";
            }else if (data[d].sequencing_platform.includes('MiSeq')){
                series_name = "MiSeq";
            }else if (data[d].sequencing_platform.includes('NextSeq')){
                series_name = "NextSeq";
            }else{
                series_name = "HiSeq";
            }
            ...
            if (data[d].type == null){
                ...
                series_name = data[d].type;
            }
            view_color = view_coloring(series_name);
        }else if (view_type == 'delivery_type'){
            if (data[d].delivery_type == null){
                series_name = "Other/undefined";
            }else{
                series_name = data[d].delivery_type;
            }
            view_color = view_coloring(series_name);
        }else if (view_type == 'sample_type'){
            if (data[d].sample_type.includes('Finished')){
                series_name = "Finished Library";
            }else if (data[d].sample_type.includes('total')){
                series_name = "total RNA";
            }else if (data[d].sample_type.includes('Tissue')){
                series_name = "Tissue";
            }else if (data[d].sample_type.includes('Genomic')){
                series_name = "Genomic DNA";
            }else if (data[d].sample_type.includes('Amplicon')){
            ...
            if (data[d].best_practice_bioinformatics == 'Yes'){
                series_name = "BP bioinformatics";
            }else if (data[d].best_practice_bioinformatics == 'No'){
                series_name = "No BP";
            }else if (data[d].best_practice_bioinformatics == 'Special'){
                series_name = "Special";
            }else{
                series_name = "Other/undefined";
            }
